...
def load_tree_sitter() -> Tuple[object, object]:
    try:
        ...
        for attr in ("language", "PY_LANGUAGE", "PYTHON_LANGUAGE", "PY_LANGUAGE"):
            if hasattr(_tsp, attr):
                PY_LANG = getattr(_tsp, attr)
                return (Parser, PY_LANG)
        if isinstance(_tsp, object):
            return (Parser, _tsp)
    except Exception:
        pass
    try:
        ...
        for finder, name, ispkg in pkgutil.iter_modules():
            pass
        raise ImportError(
            "tree_sitter Language not found in installed packages. If you installed tree-sitter runtime but did not install/compile the Python grammar, you must provide a compiled language .so/.pyd/.dylib.\n\nOption 1: install the 'tsp' package that bundles a Python grammar (you mentioned tsp==0.25.0).\nOption 2: compile tree-sitter-python into a shared library and point this script at it (modify load_tree_sitter()).\nSee https://tree-sitter.github.io/tree-sitter/using-parsers#installing-parsers for build instructions."
        )
    except Exception:
        pass
    raise ImportError(
        "Unable to import a compatible tree-sitter binding + python grammar. Try installing packages: pip install ts==0.25.2 tsp==0.25.0 or pip install tree_sitter and build the tree-sitter-python shared library."
    )
...
def ast_tokenize_strip(text: str, preserve_module_docstring: bool = True) -> str:
    ...
    for tok in g:
        ...
        if tok_type == tokenize.COMMENT:
            ...
            erow, ecol = tok.end
            is_doc = False
            for ln, eln, lc, ec in docstring_spans:
...
def main(argv: Optional[List[str]] = None):
    ...
    ap.add_argument(
        "--compare",
        action="store_true",
        help="Run a dry-run timing comparison between tree-sitter and ast/tokenize approaches before writing.",
    )
    ...
    if args.compare:
        print("Running dry-run comparison (no files will be written during timing)...")
        print("Timing tree-sitter implementation...")
        ...
        print("Timing AST+tokenize implementation...")
        ...
        ratio = elapsed_ast / elapsed_ts if elapsed_ts > 0 else float("inf")
        print(f"Speed: ast/tokenize is {ratio:.2f}x tree-sitter (lower is faster).")
    if args.dry_run:
        print("Dry-run requested; not writing files. Exiting.")
        return 0
    print("Processing files with tree-sitter and writing changes in-place...")
    changed, errors, elapsed = run_parallel(files, process_file_tree_sitter, workers=args.workers, dry_run=False)
    print(f"Done: processed {len(files)} files in {elapsed:.3f}s; updated {changed} files; errors: {len(errors)}")
    if errors:
