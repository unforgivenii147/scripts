...
class WheelBuilder:
    def __init__(
        ...
        self.max_workers = max_workers or min(cpu_count(), 8)  # Limit to 8 max to avoid overwhelming
        ...
        # Track processed packages to avoid duplicates (for serial mode)
    ...
    def build_wheel(self, dist_info_dir: Path) -> Optional[Path]:
        ...
        # Skip if already processed (for serial mode)
    ...
    def build_all(self) -> int:
        """Build wheels for all packages with optional parallel processing."""
        ...
        if self.parallel and len(dist_infos) > 1:
            print(f"🚀 Using parallel processing with {self.max_workers} workers")
            return self._build_parallel(dist_infos)
        else:
            if len(dist_infos) > 1:
    ...
    def _build_parallel(self, dist_infos: List[Path]) -> int:
        """Build wheels in parallel using multiprocessing."""
        # Reset processed packages for parallel mode
        self.processed_packages.clear()

        # Create a partial function with the builder instance
        # Note: We need to pickle the builder, so we use a wrapper
        ...
        # Use Pool with the builder instance
        with ctx.Pool(processes=self.max_workers) as pool:
            # Map the function to all dist_infos
...
Examples:
  ...
  python /path/to/script.py -o /path/to/wheels
    ...
    # Verify we're in a site-packages directory
    ...
    if args.package:
        ...
        # Always use serial for single package (parallel overhead not worth it)
