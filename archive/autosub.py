...
import requests
from autosub.constants import GOOGLE_SPEECH_API_KEY, GOOGLE_SPEECH_API_URL, LANGUAGE_CODES
from autosub.formatters import FORMATTERS
from googleapiclient.discovery import build
from progressbar import ETA, Bar, Percentage, ProgressBar


def percentile(arr, percent: float):
    arr = sorted(arr)
    k = (len(arr) - 1) * percent
    f = math.floor(k)
    c = math.ceil(k)
...
class FLACConverter:
    def __init__(self, source_path, include_before: float = 0.25, include_after: float = 0.25) -> None:
        self.source_path = source_path
        self.include_before = include_before
        self.include_after = include_after

    def __call__(self, region) -> bytes | None:
        try:
            start, end = region
            ...
            subprocess.check_output(command, stdin=pathlib.Path(os.devnull).open(encoding="utf-8"))
            return temp.read()
...
class SpeechRecognizer:
    def __init__(
        self, language: str = "en", rate: int = 44100, retries: int = 3, api_key=GOOGLE_SPEECH_API_KEY
    ) -> None:
...
class Translator:
    def __init__(self, language, api_key, src, dst) -> None:
        self.language = language
        self.api_key = api_key
    ...
    def __call__(self, sentence):
        try:
            if not sentence:
                return None
            result = self.service.translations().list(source=self.src, target=self.dst, q=[sentence]).execute()
            if (
...
def which(program: str):
    def is_exe(fpath: str) -> bool:
        return pathlib.Path(fpath).is_file() and os.access(fpath, os.X_OK)
...
def extract_audio(filename, channels=1, rate=16000):
    ...
    if not pathlib.Path(filename).is_file():
        ...
        msg = f"Invalid filepath: {filename}"
        raise Exception(msg)
    if not which("ffmpeg"):
        print("ffmpeg: Executable not found on machine.")
        msg = "Dependency not found: ffmpeg"
        raise Exception(msg)
    command = [
        "ffmpeg",
        "-y",
...
def find_speech_regions(filename: str, frame_width=4096, min_region_size=0.5, max_region_size=6):
        ...
        try:
            ...
            if not is_same_language(args.src_language, args.dst_language):
                if args.api_key:
                    ...
                    prompt = f"Translating from {args.src_language} to {args.dst_language}: "
                    ...
                    print("Error: translation requires API key")
                    return 1
        except KeyboardInterrupt:
            pbar.finish()
            pool.terminate()
            ...
            return 1
    timed_subtitles = [(r, t) for r, t in zip(regions, transcripts, strict=False) if t]
    formatter = FORMATTERS.get(args.format)
    formatted_subtitles = formatter(timed_subtitles)
    dest = args.output
    if not dest:
        base, _ext = os.path.splitext(args.source_path)
        dest = f"{base}.{args.format}"
    pathlib.Path(dest).write_bytes(formatted_subtitles.encode("utf-8"))
    print(f"Subtitles file created at {dest}")
    pathlib.Path(audio_filename).unlink()
    return 0
