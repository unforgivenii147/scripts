...
def generate_html(node: Node) -> str:
    ...
    if node.inputs:
        document.append(f"<h2>target is built using rule <tt>{html_escape(node.rule)}</tt> of</h2>")
        if len(node.inputs) > 0:
            ...
            for input, type in sorted(node.inputs):
                ...
                document.append(f'<tt><a href="?{html_escape(input)}">{html_escape(input)}</a>{extra}</tt><br>')
            document.append("</div>")
    if node.outputs:
        document.append("<h2>dependent edges build:</h2>")
        document.append("<div class=filelist>")
        for output in sorted(node.outputs):
            document.append(f'<tt><a href="?{html_escape(output)}">{html_escape(output)}</a></tt><br>')
        document.append("</div>")
    return "\n".join(document)
