...
def main() -> None:
    ...
    site_dirs = [
        Path("/data/data/com.termux/files/home/tmp/site-packages"),
    ]
    for site_dir in site_dirs:
        ...
        for dist_info in site_dir.glob("*.dist-info"):
            ...
            for p in files:
                if not p.exists() and "license" not in p.name.lower() and "/bin/" not in str(p):
    ...
    if missing_files:
        ...
        for name, files in missing_files.items():
            cprint(
                f" - {name} ({len(files)} files missing)",
                "cyan",
            )
            for f in files:
                print(Path(f).name)
