...
def save_entity(entity: dict[str, Any]) -> None:
    ...
    if entity["is_function"]:
        final_zst_path = final_py_path.with_suffix(".py.zst")
        try:
            cctx = zstd.ZstdCompressor(level=3)
            compressed_data = cctx.compress(content.encode("utf-8"))
            Path(final_zst_path).write_bytes(compressed_data)
        except Exception as e:
            print(f"Error compressing {final_zst_path}: {e}")
