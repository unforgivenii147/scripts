...
def parse_record(
    ...
    try:
        ...
        logger.debug(f"[ERROR] RECORD parse failed: {dist_info}: {exc}")
    return files
...
def main() -> None:
        ...
        for dist_info in site_dir.glob("*.dist-info"):
            ...
            if not tops:
                ...
                if not top_file.exists() or top_file.stat().st_size == 0:
                    top_file.write_text("\n".join(sorted(tops)) + "\n")
                    logger.info(f"[FIXED] {dist_info.name} → {', '.join(sorted(tops))}")
            for p in files:
                if not p.exists():
        ...
        for f, owners in file_map.items():
            if len(owners) > 1:
                ...
                if len(hashes) != 1:
                    conflicted = True
        if conflicted:
            logger.info("  ⚠ overlapping files detected — auto-cleanup unsafe")
        else:
            ordered = sorted(infos, key=lambda p: p.name)
            keep = ordered[-1]
            remove = ordered[:-1]
            logger.info(f"  keep   → {keep.name}")
            for r in remove:
                logger.info(f"  remove → {r.name} (safe candidate)")
    if empty_pkgs:
        logger.info("\n[EMPTY DISTRIBUTIONS]")
        for d in empty_pkgs:
            logger.debug(f" - {d}")
    if missing_files:
        ...
        for name, files in missing_files.items():
            logger.info(f" - {name} ({len(files)} files missing)")
            for f in files:
                logger.info(Path(f).name)
    logger.info("\nInspection complete.")
