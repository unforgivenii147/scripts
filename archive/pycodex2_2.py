...
class CodeBlockExtractor:
    ...
    def _extract_from_json(self, data, depth=0, max_depth=5) -> list[str]:
        ...
        if isinstance(data, dict):
            ...
            if any((keyword in data for keyword in ["def ", "import ", "class ", "if __name__"])):
...
class FileProcessor:
    ...
    def _save_code_blocks(self, code_blocks: list[CodeBlock], source: str) -> None:
        ...
        for block in code_blocks:
            if block.suggested_name:
                filename = block.suggested_name
            else:
                filename = f"{source_name}_block_{block.block_index:03d}.py"
        ...
        if args.url:
            ...
            if html_files:
                ...
                with Pool(processes=args.jobs) as pool:
                    results = pool.map(processor.process_file, html_files)
