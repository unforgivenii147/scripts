...
MAX_RETRIES = 1  # per-segment retry attempts
...
# CJK Unified Ideographs ranges only (no punctuation)
_CHINESE_RANGES = (
...
def is_chinese_ideograph(ch: str) -> bool:
    """Check if a single character is a Chinese ideograph (not punctuation)."""
    return any(lo <= ord(ch) <= hi for lo, hi in _CHINESE_RANGES)


def has_chinese(text: str) -> bool:
    """Check if text contains any Chinese ideographs."""
    return any(is_chinese_ideograph(ch) for ch in text)


def find_chinese_segments(text: str) -> list[tuple[int, int, str]]:
    """
    Find contiguous segments of Chinese ideographs in text.
    Segments only include actual Chinese characters, not punctuation.
...
def process_file(path: Path) -> bool:
    """
    Translate Chinese characters in `path` in-place.
    Only translates Chinese ideographs, preserving punctuation and everything else.
    ...
    # Find all lines with Chinese characters and their segments
    line_segments = {}
    ...
    for line_idx, segments in line_segments.items():
        ...
        for start, end, chinese_text in segments:
            ...
            # Translate the Chinese segment (only ideographs, no punctuation)
    ...
    # Count failed segments (only ideographs, not punctuation)
    failed_segments = 0
    for line_idx, segments in line_segments.items():
        if line_idx in done:
