...
class Marker(NamedTuple):
    ...
    def parse(cls, s: str) -> Marker:
        if " or " in s:
            raise NotImplementedError("Marker only supports `and` -- maybe repeat `marker =`?")
        ret = []
        chunks = s.split(" and ")
        for chunk in chunks:
            parts = chunk.split()
            if (
                len(parts) != 3
                or parts[0] not in _Key.__members__
                or parts[1] != "=="
                or (not parts[2].startswith('"'))
                or (not parts[2].endswith('"'))
            ):
                raise ValueError(f'invalid marker part: expected: `{_KEY_DISPLAY} == "..."` got: `{chunk}`')
...
def _parse(s: str | None, section: str) -> tuple[File, ...]:
    ...
    for path, body in zip(parts, parts):
        ...
        if bool(extract) != bool(extract_path):
            raise ValueError(f"{section}[{path}]: missing `extract` + `extract_path`")
        if extract is not None and extract not in _EXTRACT:
            raise ValueError(
                f"{section}[{path}]extract: unexpected value {extract!r}, ({', '.join(sorted(_EXTRACT.keys() - {'noop'}))})"
            )
        ...
        ret.append(File(path=path, url=url, sha256=sha256, archive=archive, group=group, markers=tuple(markers)))
    return tuple(ret)


def _filter(src: tuple[File, ...], section: str) -> tuple[File, ...]:
    env = _default_env()
    ret = [f for f in src if not f.markers or any((m.evaluate(env) for m in f.markers))]
    seen = set()
    seen_groups = set()
    for f in ret:
        if f.path in seen:
...
def _download(f: File, base: str) -> None:
    ...
    if not secrets.compare_digest(got, f.sha256):
        raise ValueError(f"{f.path}: checksum mismatch got={got!r} f.sha256={f.sha256!r}")
...
class setuptools_download(Command):
    user_options = [("download-data-files=", None, ""), ("download-scripts=", None, "")]
...
def finalize_distribution(dist: Distribution) -> None:
    dist.get_command_class("build").sub_commands.append((setuptools_download.__name__, None))
    dist.get_command_class("install").sub_commands.append((install_setuptools_download.__name__, None))
