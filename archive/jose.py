...
def encrypt(
    claims, jwk, adata="", add_header=None, alg="RSA-OAEP", enc="A128CBC-HS256", rng=get_random_bytes, compression=None
):
    ...
    if compression is not None:
        ...
        try:
            ...
            msg = f"Unsupported compression algorithm: {compression}"
            raise Error(msg)
    ...
    hash = hash_fn(_jwe_hash_str(ciphertext, iv, adata), encryption_key[: -hash_mod.digest_size / 2], hash_mod)
    (cipher, _), _ = JWA[alg]
    encryption_key_ciphertext = cipher(encryption_key, jwk)
    return JWE(*map(b64encode_url, (json_encode(header), encryption_key_ciphertext, iv, ciphertext, auth_tag(hash))))


def decrypt(jwe, jwk, adata="", validate_claims=True, expiry_seconds=None):
    header, encryption_key_ciphertext, iv, ciphertext, tag = map(b64decode_url, jwe)
    ...
    if version:
        ...
        hash = hash_fn(_jwe_hash_str(ciphertext, iv, adata, version), encryption_key[: -mod.digest_size / 2], mod=mod)
    else:
        plaintext = decipher(ciphertext, encryption_key[: -mod.digest_size], iv)
        hash = hash_fn(_jwe_hash_str(ciphertext, iv, adata, version), encryption_key[-mod.digest_size :], mod=mod)
    if not const_compare(auth_tag(hash), tag):
        msg = "Mismatched authentication tags"
        raise Error(msg)
    if "zip" in header:
        try:
            _, decompress = COMPRESSION[header["zip"]]
        except KeyError:
            msg = "Unsupported compression algorithm: {}".format(header["zip"])
            raise Error(msg)
...
def sign(claims, jwk, add_header=None, alg="HS256"):
    ...
    sig = b64encode_url(hash_fn(_jws_hash_str(header, payload), jwk["k"], mod=mod))
...
def verify(jws, jwk, alg, validate_claims=True, expiry_seconds=None):
    ...
    if not verify_fn(_jws_hash_str(jws.header, jws.payload), jwk["k"], sig, mod=mod):
...
def b64decode_url(istr):
    ...
    try:
        ...
        msg = f"Unable to decode base64: {e}"
        raise Error(msg)
...
def hmac_verify(s, key, sig, mod=SHA256):
    hmac = HMAC.new(key, digestmod=mod)
    hmac.update(s)
    return const_compare(hmac.digest(), sig)
...
def const_compare(stra, strb):
    if len(stra) != len(strb):
        return False
    res = 0
    for a, b in zip(stra, strb, strict=False):
        res |= ord(a) ^ ord(b)
    return res == 0
...
class _JWA:
    ...
    def _compound_from_key(self, key):
        ...
        try:
            ...
            pass
        msg = f"Unsupported algorithm: {key}"
        raise Error(msg)


JWA = _JWA()
COMPRESSION = {"DEF": (zlib.compress, zlib.decompress)}
...
def _check_expiration_time(now, expiration_time):
    if now >= expiration_time:
        msg = "Token expired at {}".format(_format_timestamp(expiration_time))
        raise Expired(msg)


def _check_not_before(now, not_before):
    if not_before > now:
        msg = f"Token not valid until {_format_timestamp(not_before)}"
        raise NotYetValid(msg)
...
def _cli():
    ...
    handler_args = [getattr(args, k) for k in inspect.getargspec(handler).args]
