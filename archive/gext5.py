...
ARCHIVE_EXTENSIONS = (".whl", ".zip", ".tar.gz", ".tgz", ".tar.xz", ".tar")
...
def process_archive(path: Path) -> list[dict[str, Any]]:
    ...
    elif any((path.name.endswith(ext) for ext in [".tar", ".tar.gz", ".tgz", ".tar.xz"])):
        mode_map = {".tar.gz": "r:gz", ".tgz": "r:gz", ".tar.xz": "r:xz", ".tar": "r"}
