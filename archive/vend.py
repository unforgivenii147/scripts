...
extras = [
    Extra(name="amatch", description="Approximate matches virtual table"),
    Extra(name="anycollseq", description="Fake fallback collating function for any unknown collating sequence"),
    Extra(
        name="appendvfs",
    ...
    Extra(name="base64", description="Convert either direction between base64 blob and text"),
    Extra(name="base85", description="Convert either direction between base85 blob and text"),
    Extra(
        name="btreeinfo",
    ...
    Extra(name="compress", description="SQL compression functions", lib_zlib=True),
    Extra(name="csv", description="A virtual table for reading CSV files", doc="csv.html"),
    Extra(name="decimal", description="Routines to implement arbitrary-precision decimal math"),
    Extra(name="eval", description="Implements SQL function eval() which runs SQL statements recursively"),
    Extra(
        name="fileio",
    ...
    Extra(name="nextchar", description="Finds all valid 'next' characters for a string given a vocabulary"),
    Extra(name="noop", description="Implements noop() functions useful for testing"),
    Extra(name="prefixes", description="Table valued function providing all prefixes of a string"),
    Extra(name="randomjson", description="Generates random json objects"),
    Extra(name="regexp", description="Compact reasonably efficient posix extended regular expression matcher"),
    Extra(name="rot13", description="rot13 function and collating sequence"),
    Extra(name="sha1", description="SHA1 hash and query results hash"),
    Extra(name="shathree", description="SHA3 hash and query results hash"),
    Extra(name="spellfix", description="Search a large vocabulary for close matches", doc="spellfix1.html"),
    Extra(
        name="sqlar",
    ...
    Extra(name="uint", description="UINT collating sequence"),
    Extra(name="unionvtab", description="Virtual table combining underlying tables from other databases"),
    Extra(name="uuid", description="uuid functions"),
    Extra(name="vfsstat", description="VFS shim tracking call statistics"),
    Extra(
        name="vtablog",
        description="Virtual table printing diagnostic information for interactive analysis and debugging",
    ),
    Extra(name="zipfile", doc="zipfile.html", description="Read/Write access to simple archives", lib_zlib=True),
    Extra(name="zorder", description="Functions for z-order (Morton code) transformations"),
    Extra(
        name="vec1",
...
def c_quote(value: str, quote: str = '"') -> str:
    assert '"' not in value
    return quote + value + quote


def make_windows_resource(manifest_filename: str | None, **fields) -> str:
    ...
    for line in source.splitlines():
        if line.strip().startswith("IDI_SQLITE ICON"):
            out.append('IDI_SQLITE ICON "sqlite3\\\\art\\\\sqlite370.ico"')
            continue
...
def resource_file(build_dir: Path, compiler: Compiler, extra: Extra) -> str:
...
def get_version() -> None:
    pat = "#define\\s+(" + "|".join(Version.keys()) + ')\\s+"(.*)"\\s*$'
...
def do_build(what: set[str], verbose: bool, fail_fast: bool = False) -> None:
    get_version()
    ...
    if compile_extra_preargs:
        print(f"Checking compile_extra_preargs={compile_extra_preargs!r}")
        try:
    ...
    if link_extra_preargs:
        print(f"Checking link_extra_preargs={link_extra_preargs!r}")
        try:
    ...
    macros.extend([("SQLITE_USE_URI", 1), ("SQLITE_THREADSAFE", 2)])
        ...
        for extra in extras:
            ...
            match extra.type:
                case "extension":
                    ...
                    out_name = (
                        f"{extra.name}{(compiler.shared_lib_extension if sys.platform != 'darwin' else '.dylib')}"
                    )
                    try:
...
def get_compile_check(build_dir: Path):
    with open(build_dir / "compile_check.c", mode="wt") as f:
        ...
        for item in ("char", "int", "long", "long long", "int*", "void*"):
            f.write(f' printf("sizeof({item})=%d\\n", (int)sizeof({item}));' + "\n")
