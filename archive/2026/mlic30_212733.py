...
def process_file(args: Tuple[Path, int, int, Set[str]]) -> Tuple[Path, Dict[str, List[Tuple[int, int]]]]:
...
def find_repeated_strings(
    directory: Path, min_lines: int = 2, min_chars: int = 10, max_workers: int = None, extensions: Set[str] = None
...
def remove_strings_from_files(
    repeated_strings: Dict[str, List[Tuple[Path, List[Tuple[int, int]]]]], string_numbers: List[int] = None
):
    """
...
def main():
    ...
    if args.extensions:
        ...
        if not all(ext.startswith(".") for ext in extensions):
            print("Warning: Some extensions don't start with '.', adding it automatically...")
